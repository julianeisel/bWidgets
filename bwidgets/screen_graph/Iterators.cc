...
{
	return (is_root != other.is_root) || (is_root ? (node != other.node) : (node_iter != other.node_iter));
}
...
PreOrderIterator& PreOrderIterator::operator++()
{
	...
	if (!childs || childs->empty()) {
		...
		if (hasExceededLastSibling()) {
			...
			for (auto parent_iter = ancestors.rbegin(); hasExceededLastSibling(); ++parent_iter, ancestors.pop_back()) {
