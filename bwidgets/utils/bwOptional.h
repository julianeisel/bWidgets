...
public:
	...
	template<typename _U, typename std::enable_if<
	                 ...
	                 std::is_convertible<const _U&, _T>::value &&
	...
	template<typename _U, typename std::enable_if<
	                 !std::is_same<_T, _U>::value &&
	                 std::is_constructible<_T, _U&&>::value &&
	...
	template<typename ... Args, typename std::enable_if<
	             std::is_constructible<_T, Args...>::value
	             , bool>::type = false>
	constexpr explicit bwOptional(in_place_t, Args&&... args) :
	    Base(in_place, std::forward<Args>(args)...)
	{}

	template<typename  _U, typename... Args, typename std::enable_if<
	...
	template<typename _U = _T, typename std::enable_if<
	             std::is_constructible<_T, _U&&>::value &&
	             !std::is_same<std::decay<_U>, in_place_t>::value &&
	             !std::is_same<std::decay<_U>, bwOptional<_T>>::value &&
	             std::is_convertible<_U&&, _T>::value
	             , bool>::type = false>
	constexpr bwOptional(_U&& value) :
	    Base(in_place, std::forward<_U>(value))
	{}
	template<typename _U = _T, typename std::enable_if<
	             std::is_constructible<_T, _U&&>::value &&
	             !std::is_same<std::decay<_U>, in_place_t>::value &&
	             ...
	             !std::is_convertible<_U&&, _T>::value
	             , bool>::type = false>
	constexpr explicit bwOptional(_U&& value) :
	    Base(in_place, std::forward<_U>(value))
	{}
	...
	/*constexpr*/ // While this function itself could be constexpr, it calls addressof which is not constexpr in C++11.
	...
	{
		// Need to squeeze this into a single instruction to workaround bad C++11 constexpr support.
		return (assert(has_value() && "attempt to get value of a disengaged optional"), std::move(this->get()));
	}
	/*constexpr*/ _T&& operator*() &&
	{
		return (assert(has_value() && "attempt to get value of a disengaged optional"), std::move(this->get()));
	...
	constexpr _T value_or(_U&& or_value) const&
	{
...
template<typename _T>
constexpr bool operator==(const bwOptional<_T>& opt, nullopt_t)
{
	return !opt;
}
template<typename _T>
constexpr bool operator==(nullopt_t, const bwOptional<_T>& opt)
{
	return !opt;
}
template<typename _T, typename _U>
...
template<typename _T>
constexpr bwOptional<typename std::decay<_T>::type> make_optional(_T&& value)
{
	return bwOptional<typename std::decay<_T>::type>(std::forward<_T>(value));
}
template<typename _T, typename... Args>
constexpr bwOptional<_T> make_optional(Args&&... args)
{
...
template<typename _T, typename std::enable_if<
             ...
             // is_swappable is C++17 only. We could implement it if really needed...
...
using __enable_hash_helper_impl = _Type;
template<typename _Type, typename _KeyType>
using __enable_hash_helper = __enable_hash_helper_impl<_Type, decltype(hash<_KeyType>()(declval<_KeyType>()))>;
