...
typedef struct {
  ...
  GLubyte* vertex_data;
  uint16_t
      unassigned_attrib_bits; /* which attributes of current vertex have not been given values? */
...
void immAttrib4ub(
    uint attrib_id, unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
  Gwn_VertAttr* attrib = imm.vertex_format.attribs + attrib_id;
#if TRUST_NO_ONE
  assert(attrib_id < imm.vertex_format.attr_len);
...
#  if TRUST_NO_ONE
#    define GET_UNIFORM \
      const Gwn_ShaderInput* uniform = GWN_shaderinterface_uniform(imm.shader_interface, name); \
      assert(uniform);
#  else
#    define GET_UNIFORM \
      const Gwn_ShaderInput* uniform = GWN_shaderinterface_uniform(imm.shader_interface, name);
...
{
  const Gwn_ShaderInput* uniform = GWN_shaderinterface_uniform_builtin(imm.shader_interface,
                                                                       GWN_UNIFORM_COLOR);
#if TRUST_NO_ONE
  assert(uniform != NULL);
#endif
