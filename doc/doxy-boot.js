/*
 * From https://github.com/Velron/doxygen-bootstrapped/blob/master/doxy-boot.js
 * Modifications copyright (c) 2017 Julian Eisel
 *
 * Copyright (c) 2014 Velron (https://github.com/Velron)
 *
...
$( document ).ready(function() {
    ...
    $(".tablist > li > a").css('color', '#FFF');
    $("li.current").addClass("active");
    $(".nav-pills > li.active > a").css('background-color', 'rgba(0, 0, 0, 0.2)');
    $(".nav > li:not(.active) > a").hover(function(e) {
      $(this).css('background-color', e.type == 'mouseenter' ? 'rgba(255, 255, 255, 0.1)' : '#5373B4');
    });
    $("iframe").attr("scrolling", "yes");

    $("#nav-path").addClass("content-wrapper");
  ...
  var nav_container = $('<div class="row content-wrapper"></div>');
  ...
  navarea = $(nav_container).parent().prepend('<div id="navarea"></div>');
  $(nav_container).prependTo($('div#navarea'));
  // $('div#navarea').css('width', '1170px');
  // $('div#navarea').css('margin', '0 auto');
  ...
  $('#MSearchResultsWindow')
    ...
    .addClass('panel panel-default content-wrapper dropdown-menu')
    .attr('role', 'menu')
    .append(
  ...
  $("div.header").addClass('content-header');
  $("div.header").removeClass('header');
  // $("div.content-header").addClass('page-content');
  // $("div.contents").addClass('page-content');
  ...
  $('body').css('background-color', '#EEF1F7');
  $('body').css('color', '#555');

  $('.nav-justified > li').css('float', 'left');
  $('.nav-justified > li').css('width', 'auto');

  $('body').append('<div id="page-content" class="content-wrapper"></div>');
  $('div.contents').prependTo($('#page-content'));
  $('div.content-header').prependTo($('#page-content'));

  if(document.getElementsByClassName("summary").length > 0) {
  // if($('div.content-header').find('summary') != null) {
    $('div.page-header').css('margin', '40px 0');
  }
  else {
