...
{
	{
		...
		::testing::StaticAssertTypeEq<decltype(opt.value()), TestUtilClasses::SimpleFunctionWithVariants&>();
	}
	{
...
{
	{
		...
		::testing::StaticAssertTypeEq<decltype(opt.value()), const TestUtilClasses::SimpleFunctionWithVariants&>();
	}
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt(in_place);
		MAYBE_STATIC_ASSERT(opt.value().foo() == TestUtilClasses::SimpleFunctionWithVariants::CONST_LREF);
	}
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt;
		EXPECT_THROW(opt.value(), bad_optional_access);
...
TEST(bwOptional, value_rval_non_const)
{
	{
		bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt;
		::testing::StaticAssertTypeEq<decltype(std::move(opt).value()), TestUtilClasses::SimpleFunctionWithVariants&&>();
	}
	{
		bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt(in_place);
		EXPECT_EQ(std::move(opt).value().foo(), TestUtilClasses::SimpleFunctionWithVariants::NON_CONST_RREF);
	}
	{
		bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt;
...
TEST(bwOptional, value_rval_const)
{
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt;
		::testing::StaticAssertTypeEq<decltype(std::move(opt).value()), const TestUtilClasses::SimpleFunctionWithVariants&&>();
	}
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt(in_place);
		EXPECT_EQ(std::move(opt).value().foo(), TestUtilClasses::SimpleFunctionWithVariants::CONST_RREF);
	}
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt;
		EXPECT_THROW(std::move(opt).value(), bad_optional_access);
...
public:
	...
	InitListConstructible(std::initializer_list<int> ilist) : _a(ilist.begin()[0]), _b(ilist.begin()[1]) {}
	~InitListConstructible() {tot_ctor_calls++;}
