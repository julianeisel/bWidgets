...
template<typename _Optional> void test_constructor_default_constexpr()
{
...
TEST(bwOptional, constructor_copy)
{
  ...
  static_assert(!std::is_trivially_copy_constructible<
                    bwOptional<TestUtilClasses::NonTrivialConstructors>>::value,
                "");
...
TEST(bwOptional, constructor_move)
{
  ...
  static_assert(std::is_copy_constructible<bwOptional<TestUtilClasses::NoMoveConstructor>>::value,
                "");
  // A type can be move-constructible if it's copy-constructible, even if move constructor is
  // deleted.
  ...
  }
  {
    ...
    class SomeObject {
     public:
      ...
      SomeObject(nullopt_t) : _is_nullopt(true)
      {
        tot_instances++;
      }
      SomeObject(int x, int y, int z) : _x(x), _y(y), _z(z)
      {
        tot_instances++;
      }
    };
    ...
    }
    {
      class ConstexprCtor {
       public:
        int _x, _y, _z;
        constexpr ConstexprCtor(int x, int y, int z) : _x(x), _y(y), _z(z)
        {
        }
      };
