...
TEST(bwOptional, operator_assign_nullopt)
{
	...
	{
		bwOptional<TestUtilClasses::NonTrivialConstructors> opt{TestUtilClasses::NonTrivialConstructors(42)};
...
TEST(bwOptional, operator_assign_copy)
{
	...
	static_assert(!std::is_copy_assignable<bwOptional<TestUtilClasses::NoCopyAssignmentOperator>>::value, "");
}
...
TEST(bwOptional, operator_assign_move)
{
	...
	static_assert(!std::is_move_assignable<bwOptional<TestUtilClasses::NoMoveAssignmentOperator>>::value, "");
}
...
struct UndefinedAddressOfOperator
{
	...
	constexpr int foo() const {return 18;}
};
...
TEST(bwOptional, operator_arrow_const)
{
	...
	}
	{
		...
		::testing::StaticAssertTypeEq<decltype(opt.operator->()), const TestUtilClasses::SimpleFunction*>();
	}
	{
...
TEST(bwOptional, operator_dereference_rval_const)
{
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt(in_place);
		MAYBE_STATIC_ASSERT(std::move(*opt).foo() == TestUtilClasses::SimpleFunctionWithVariants::CONST_RREF);
	}
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunction> opt;
		::testing::StaticAssertTypeEq<decltype(*std::move(opt)), const TestUtilClasses::SimpleFunction&&>();
	}
	{
		constexpr bwOptional<TestUtilClasses::SimpleFunctionWithVariants> opt;
		EXPECT_DEATH(std::move(*opt).foo(), "attempt to get value of a disengaged optional");
